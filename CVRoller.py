...
def readdata(file,secname):
    ...
    sheet['attribute'] = sheet['attribute'].fillna('raw')
    ...
    for sec in sheet['section'].unique():
        ...
        if (secdata['id'].isnull().sum()) == seclength:
...
def buildmd(vsd,data,secframedef,secglue,itemwrapperdef,type):
    ...
    for sec in vsd:
        ...
        if type == 'pdf':
            ...
            s['meat'] = pypandoc.convert_text(s['meat'],'latex',format='md',encoding='utf-8',extra_args=['--wrap=none'])
            try: 
                s['subtitle'] = pypandoc.convert_text(s['subtitle'],'latex',format='md',encoding='utf-8',extra_args=['--wrap=none'])
        ...
        #no-wrap to avoid weird line breaks around character 70.
        if type == 'pdf':
            s['out'] = pypandoc.convert_text(s['out'],'latex',format='md',encoding='utf-8',extra_args=['--wrap=none'])
        '''    
...
with open('cvlayout.txt','r') as structfile:
...
for v in versions:             
    ...
    if versions[v]['type'] == 'html':
        ...
        for sec in vsd:
            ...
            #Drop nans
            attlist = [x for x in attlist if str(x) != 'nan']
    ...
    #####################
        ...
        pypandoc.convert_text(cv,'md',format='md',outputfile=versions[v]['out'],encoding='utf-8',extra_args=['--wrap=none'])
